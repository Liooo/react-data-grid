...
function DataGrid(props, ref) {
  ...
  const [shouldFocusCell, setShouldFocusCell] = react.useState(false);
  const [previousRowIdx, setPreviousRowIdx] = react.useState(-1);
  ...
  const [prevSelectedPosition, setPrevSelectedPosition] = react.useState(selectedPosition);
  ...
  const focusCellOrCellContent = react.useCallback(() => {
  ...
  useLayoutEffect(() => {
    if (!selectedCellIsWithinSelectionBounds || isSamePosition(selectedPosition, prevSelectedPosition)) {
